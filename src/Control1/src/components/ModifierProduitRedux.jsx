...
function ModifierProduitRedux() {
  ...
  const catalogue = store.getState();
  ...
  function rechercheparCode(code) {
    if (code === "") {
      ...
      // setCodeRech(code);
      const prdAchercher = catalogue.find(
        (prd) => prd.code.toLowerCase() === code.toLowerCase()
      );
  ...
  function handleSubmit(e) {
    ...
    if (isEdit === false) {
      const prdTrouve = rechercheparCode(produit.code);
    ...
    <>
      ...
      <form onSubmit={(e) => handleSubmit(e)}>
        ...
        <input
          type="submit"
          value={
            isEdit === false ? "Rechercher le produit" : "Modifier le produit"
          }
        />
      </form>
      <div></div>
...
export default ModifierProduitRedux;
